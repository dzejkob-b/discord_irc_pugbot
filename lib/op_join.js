...
class OpJoin {
    ...
    exec(channelKey) {
        let operRef = this.parent, catRef, wRef, result, cStk = this.parent.cStk, partRef = this.parent.partRef, privPartRef = this.parent.privPartRef;
        ...
        if (operRef.getAction(channelKey).logicState != 0) {
            ...
            if (!catRef) {
                ...
                partRef.noticeMessage(operRef, WordCo.cre().text('Please join channel ').channelLink(operRef.botRef.channels[catRef.channelKey], '', '').text(' first to join ').texth(catRef.flag).text(' pug!'));

            } else if ((wRef = catRef.testParticipantTimeout(partRef)) != null) {
                partRef.noticeMessage(operRef, wRef);

            } else if (operRef.botRef.ircAuthUserOnly && !partRef.getAuthKeyRelevant()) {
                ...
                if (result == -1) {
                    ...
                    operRef.msgRef.sendMsg(channelKey, WordCo.cre().text('Player ').texth(partRef.nick).text(' joined to ').texth(catRef.flag).text(' pug.'));
                    ...
                    if (cmd && cmd[0] == '-') {
                        ...
                        if (['tag', 'join', 'j'].includes(cmdStripped)) {
                            ...
                            operRef.supplyCommandAny(catRef.channelKey, partRef, text, privPartRef);

                        } else {
                            partRef.noticeMessage(operRef, WordCo.cre().text(`Subcommand ${cmdStripped} is unknown or invalid.`));
                        }
                    }
                }
