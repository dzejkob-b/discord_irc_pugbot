		...
		operRef.getUser(
			...
			(cPartRef) => {
				...
				if (nFail) {
					...
					(cPartRef == null || cPartRef.getAuthKey() == false) &&
					nBanMask.length == 0
				) {
					partRef.noticeMessage(
						operRef,
						WordCo.cre()
							.text("Cannot ban user ")
							.texth(banNick)
							.text(
								"! User dont have discord ID or irc ACCOUNT and ban mask is not specified."
							)
					);
				} else {
					...
					operRef.banUsers[banKey] = {
						...
						by: partRef.getClone(),
						partRef:
							cPartRef != null && nBanMask.length == 0
								? cPartRef.getClone()
								: null,
					};
					...
					if (nBanDuration == 0) {
						wRef
							.text("User ")
							.texth(cPartRef.nick)
							.text(" was permanently banned by ")
							.texth(partRef.nick)
							.text(". Reason: ")
							.texth(nBanReason);
					} else {
					...
					operRef.msgRef.sendMsg(channelKey, wRef); // !! NOT privPartRef - public message
				}
			},
			partRef.type
		);
