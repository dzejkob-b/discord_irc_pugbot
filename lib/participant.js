...
class Participant {
    ...
    static fromJSON(input) {
        ...
        ["type", "id", "nick", "channel", "tag", "forceIndex", "whois", "whoisTime", "offlineTime", "authLevel", "avgPick", "avgTotPick"].forEach((c) => {
    ...
    compareEqual(partRef) {
        if (this.type == 0) {
            ...
            if (this.whois && this.whois["account"] && partRef.whois && partRef.whois["account"]) {
                return this.type == partRef.type && this.whois["account"] == partRef.whois["account"];
    ...
    matchStatsRow(row) {
        ...
        } else if (this.type == 1 && this.whois && this.whois["account"] == row['ircAuth']) {
            return true;
