class WordCo {
    ...
    isBlank() {
        ...
        while (idx < this.phrase.length) {
            if (this.phrase[idx]['t'] == 'prefix' || this.phrase[idx]['t'] == 'suffix' || this.phrase[idx]['t'] == 'sep') {
                // blank
    ...
    normalizeDiscord(value) {
        ...
        return typeof value == 'string' ? value.replace(/`/g, '\\`').replace(/\*/g, '\\*').replace(/_/g, '\\_').replace(/~/g, '\\~') : value;
    }
            ...
            switch (c['t']) {
                ...
                case 'channelLink' :
                    if (c['input']['channelDiscord']) {
                        out += c['prefix'] + '<#' + c['input']['channelDiscord'] + '>' + c['suffix'];
                    } else {
                ...
                case 'textNick' :
                    if (c['bold']) {
                        if (out.substr(out.length - 2) == '**') out += ' ';
                        out += '**' + (c['discordId'] ? ('<@' + c['discordId'] + '>') : this.normalizeDiscord(c['input'])) + '**';
                    } else {
                        out += (c['discordId'] ? ('<@' + c['discordId'] + '>') : this.normalizeDiscord(c['input']));
                    }
                    break;
            ...
            switch (c['t']) {
                ...
                case 'color' :
                    if ((useColor = this.getHtmlColor(c['color'])) != false) {
                        out += c['bold'] ? ('<strong style="color : ' + useColor + '">' + c['input'] + '</strong>') : ('<span style="color : ' + useColor + '">' + c['input'] + '</span>');

                    } else {
                        out += c['bold'] ? ('<strong>' + c['input'] + '</strong>') : ('<span>' + c['input'] + '</span>');
                    }
                    break;
