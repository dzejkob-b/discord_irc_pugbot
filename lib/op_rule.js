...
class OpRule {
    ...
    exec(channelKey) {
        ...
        if (!ruleNumb) {
            partRef.noticeMessage(operRef, WordCo.cre().text('Please specify rule number!'));
                ...
                if (userName = cStk.pop()) {
                    operRef.getUser(userName, (cPartRef) => {
                        ...
                        if (cPartRef == null) {
                            ...
                            operRef.msgRef.sendMsgArrayPrep(channelKey, ntfList, cPartRef);
                        }

                    });

                } else {
                    operRef.msgRef.sendMsgArrayPrep(channelKey, mList, privPartRef);
                }

            } else {
                operRef.msgRef.sendMsg(channelKey, WordCo.cre().text('No such rule ').texth(ruleNumb, true).text('!'), privPartRef);
            }
        }
    }
}
