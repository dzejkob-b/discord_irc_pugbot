...
import IrcUsers from "./irc_users.js";
import DiscordUsers from "./discord_users.js";
import {
	formatFromDiscordToIRC,
	formatFromIRCToDiscord,
} from "./formatting.js";
import { exit, run_again } from "./cli.js";
...
class Bot {
	constructor(options) {
		...
		this.captainPicking = options.captainPicking
			? options.captainPicking
			: "random";
	...
	connectDiscord() {
		const discordConfig = this.sources.discord;
		if (discordConfig && discordConfig.enabled) {
			...
			this.discord.login(discordConfig.token);
		}
	}

	connectIrc() {
		const ircConfig = this.sources.irc;
		if (ircConfig && ircConfig.enabled) {
			this.ircNickColor = this.sources.irc.nickColor !== false; // default to true
			// this.ircFloodDelay = options.sources.irc.floodDelay && !isNaN(parseInt(options.sources.irc.floodDelay)) ? parseInt(options.sources.irc.floodDelay) : 1000;
			// this.ircAuthUserOnly = options.sources.irc.authUserOnly ? true : false;

			const ircOptions = {
				userName: ircConfig.nickname,
				realName: ircConfig.nickname,
				channels: [],
				floodProtection: true,
				floodProtectionDelay: ircConfig.floodDelay,
				retryCount: 10,
				autoRenick: true,
				autoRejoin: true,
			...
			if (ircOptions["channels"].length > 0) {
				logger.info(
					"Connecting to IRC: " + ircConfig.server + " as " + ircConfig.nickname
				);
	...
	attachListeners() {
		if (this.discord != null) {
			...
			this.discord.on("presenceUpdate", (oldPres, newPres) => {});

			this.discord.on("messageCreate", (message) => {
				this.channelDisUsers.refreshKnownUsers();

				this.channelDisUsers.getUserId(message.author.id, (partRef) => {
					let baseText;
		...
		if (this.ircClient != null) {
			...
			this.ircClient.on("registered", (message) => {
				...
				if (ircConfig.authName && ircConfig.authPassword) {
					var authCmd;

					authCmd = [];
					authCmd.push("AUTH");
					authCmd.push(ircConfig.authName);
					authCmd.push(ircConfig.authPassword);
					...
					authCmd.push(
						"AUTH " + ircConfig.authName + " " + ircConfig.authPassword
					);
				...
				if (ircConfig.mode) {
					...
					modeCmd.push("MODE");
					modeCmd.push(ircConfig.nickname);
					modeCmd.push(ircConfig.mode);
			...
			this.ircClient.on("message", (nick, to, text, message) => {
				...
				if (to == this.sources.irc.nickname) {
					// pm message
			...
			this.ircClient.on("kick", (channelName, nick, by, reason, message) => {
				...
				if (channelKey) {
					...
					this.sendExactToDiscord(
						channelKey,
						`*${nick}* was kicked from channel (${reason}) by *${by}*`
					);
				} else {
					// !! logger.warn("Cannot find channelKey from IRC: `" + ircChannel + "` (Irc part)");
				}
	...
	logicLoop() {
		if (this.sources.irc && this.sources.irc.enabled) {
			let partRef = this.channelIrcUsers.getUser(this.sources.irc.nickname);
			if (partRef) {
				// bot is in ...
			}
		}
	...
	getDiscordNicknameOnServer(user, guild) {
		if (guild) {
			var cPartRef = this.channelDisUsers.getUserId(user.id);

			if (cPartRef) {
				return cPartRef.nick;
			}
		}
	...
	parseText(message) {
		...
		const text = message.mentions.users.reduce((content, mention) => {
			const displayName = this.getDiscordNicknameOnServer(
				mention,
				message.guild
			);
		...
		return text
			...
			.replace(/<#(\d+)>/g, (match, channelId) => {
				const channel = this.discord.channels.cache.find((ch) => {
	...
	ignoredDiscordUser(user) {
		return this.ignoreUsers.discord.some(
			(i) => i.toLowerCase() === user.toLowerCase()
		);
	}
	...
	getDiscordStatus() {
		return this.discord != null && this.discord.ws != null
			? this.discord.ws.status
			: 10;
	}
	...
	sendToIRC(channelKey, message) {
		...
		if (
			typeof this.channels[channelKey] != "undefined" &&
			typeof this.channels[channelKey]["channelIrc"] != "undefined"
		) {
			...
			const nickname = this.getDiscordNicknameOnServer(author, fromGuild);
	...
	getDiscordChannelByIrc(ircChannel) {
		for (var channelKey in this.channels) {
			if (
				typeof this.channels[channelKey]["channelIrc"] != "undefined" &&
				this.channels[channelKey]["channelIrc"] == ircChannel
			) {
	...
	getIrcChannelByDiscord(channelDiscord) {
		for (var channelKey in this.channels) {
			if (
				typeof this.channels[channelKey]["channelDiscord"] != "undefined" &&
				this.channels[channelKey]["channelDiscord"] == channelDiscord
			) {
	...
	getChannelKeyByIrc(ircChannel) {
		for (var channelKey in this.channels) {
			if (
				typeof this.channels[channelKey]["channelIrc"] != "undefined" &&
				this.channels[channelKey]["channelIrc"].toLowerCase() ==
					ircChannel.toLowerCase()
			) {
				return channelKey;
			}
		}
	...
	getChannelKeysReadable(keys) {
		...
		for (var channelKey of keys) {
			if (this.channels[channelKey]) {
				...
				result += this.channels[channelKey]["readableKey"]
					? this.channels[channelKey]["readableKey"]
					: channelKey;
			}
		}
	...
	getDiscordChannel(channelKey) {
		...
		if (
			...
			return this.getDiscordChannelById(
				this.discord.channels.cache.find((ch) => {
					return ch.id == this.channels[channelKey]["channelDiscord"];
				})
			);
		}
	...
	getDiscordChannelById(channelId) {
		var discordChannel = this.discord.channels.cache.find((ch) => {
			return ch.id == channelId;
		});
	...
	sendToDiscord(channelKey, author, text) {
		...
		if (
			...
			var discordChannel = this.getDiscordChannelById(
				this.channels[channelKey]["channelDiscord"]
			);
