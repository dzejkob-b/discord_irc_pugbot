...
class Bot {
    constructor(options) {
        ...
        if ((!this.channels || !this.channels.length) && (options.channelDiscord || options.channelIrc)) {
        ...
        if (this.discord != null) {
            ...
            this.discord.on('message', (message) => {
                if (message.channel) {
                    ...
                    const channelKey = this.getChannelKeyByDiscord(message.channel.id);
                    let partRef = null;
        ...
        if (this.ircClient != null) {
            ...
            this.ircClient.on('message', (nick, to, text, message) => {
                ...
                if (to == this.nickname) {
                    ...
                    const channelKey = this.getChannelKeyByIrc(to);
            ...
            this.ircClient.on('nick', (oldNick, newNick, channels) => {
                channels.forEach((channelName) => {
                    ...
                    const channelKey = this.getChannelKeyByIrc(ircChannel);
    ...
    sendToIRC(channelKey, message) {
        if (!this.ircClient || !this.isReady() || !channelKey) {
            return false;
        }
    ...
    sendToDiscord(channelKey, author, text) {
        if (!this.discord || !this.isReady() || !channelKey) {
            return false;
        }
        ...
        if (typeof this.channels[channelKey] != 'undefined' && typeof this.channels[channelKey]['channelDiscord'] != 'undefined') {
            ...
            const discordChannel = this.getDiscordChannelById(this.channels[channelKey]['channelDiscord']);
