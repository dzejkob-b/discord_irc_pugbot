...
class Bot {
    constructor(options) {
        ...
        this.ircFloodDelay =
            options.ircFloodDelay && !isNaN(parseInt(options.ircFloodDelay))
                ? parseInt(options.ircFloodDelay)
                : 1000;
        ...
        this.discordDisable =
            typeof options.discordDisable == 'undefined'
                ? false
                : options.discordDisable;
        this.ircDisable =
            typeof options.ircDisable == 'undefined'
                ? false
                : options.ircDisable;
        ...
        this.formatIRCText =
            this.format.ircText || '<{$displayUsername}> {$text}';
        this.formatURLAttachment =
            this.format.urlAttachment ||
    ...
    connect() {
        ...
        if (!this.ircDisable) {
            ...
            if (ircOptions['channels'].length > 0) {
                logger.info(
                    'Connecting to IRC: ' + this.server + ' as ' + this.nickname
                );
        ...
        if (this.discord != null) {
            ...
            this.discord.on('disconnect', () => {
                if (this.discord.ws.connection.ratelimit.resetTimer) {
                    clearTimeout(
                        this.discord.ws.connection.ratelimit.resetTimer
                    );
                }
            });
            ...
            this.discord.on('message', (message) => {
                let partRef = this.channelDisUsers.getUserId(message.author.id),
                    baseText;
                ...
                if (
                    ...
                    var channelKey = this.getChannelKeyByDiscord(
                        message.channel.id
                    );
                    ...
                    if (channelKey) {
                        ...
                        logger.warn(
                            'Cannot find channelKey from Discord: `' +
                                message.channel.id +
                                '` (Discord message)'
                        );
                    }
        ...
        if (this.ircClient != null) {
            this.ircClient.on('registered', (message) => {
                ...
                if (this.ircAuthName && this.ircAuthPassword) {
                    ...
                    authCmd.push(
                        'AUTH ' + this.ircAuthName + ' ' + this.ircAuthPassword
                    );
            ...
            this.ircClient.on('message', (nick, to, text, message) => {
                ...
                if (to == this.nickname) {
                    ...
                    if (channelKey) {
                        ...
                        logger.warn(
                            'Cannot find channelKey from IRC: `' +
                                ircChannel +
                                '` (Irc message)'
                        );
                    }
            ...
            this.ircClient.on('nick', (oldNick, newNick, channels) => {
                channels.forEach((channelName) => {
                    ...
                    this.channelIrcUsers.changeUserNick(
                        ircChannel,
                        oldNick,
                        newNick
                    );
    ...
    getIrcChannelByDiscord(channelDiscord) {
        for (var channelKey in this.channels) {
            if (
                typeof this.channels[channelKey]['channelDiscord'] !=
                    'undefined' &&
    ...
    sendToDiscord(channelKey, author, text) {
        ...
        if (
            ...
            if (!discordChannel) {
                logger.info(
                    'Discord channel not found for irc channel: ',
                    channelKey
                );
                return false;
            }
            ...
            const withAuthor = Bot.substitutePattern(
                this.formatDiscord,
                patternMap
            );
    ...
    sendExactToIRC(channelKey, text) {
        ...
        if (
            ...
            logger.debug(
                'Sending special message to IRC',
                text,
                '->',
                ircChannel
            );
