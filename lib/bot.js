...
class Bot {
    ...
    connectIrc() {
        ...
        if (ircConfig && ircConfig.enabled) {
            ...
            if (ircOptions['channels'].length > 0) {
                logger.info('Connecting to IRC: ' + ircConfig.server + ' as ' + ircConfig.nickname);

                this.ircClient = new irc.Client(ircConfig.server, ircConfig.nickname, ircOptions);
        ...
        if (this.ircClient != null) {
            ...
            this.ircClient.on('registered', (message) => {
                ...
                if (ircConfig.authName && ircConfig.authPassword) {
                    ...
                    authCmd.push("AUTH " + ircConfig.authName + " " + ircConfig.authPassword);
    ...
    parseText(message) {
        ...
        const text = message.mentions.users.array().reduce((content, mention) => {
            ...
            const displayName = this.getDiscordNicknameOnServer(mention, message.guild);
        ...
        return text
            ...
            .replace(/<#(\d+)>/g, (match, channelId) => {
                const channel = this.discord.channels.cache.find(ch => { return ch.id == channelId; });
    ...
    getDiscordStatus() {
        return this.discord != null && this.discord.ws != null ? this.discord.ws.status : 10;
    }
    ...
    getDiscordChannel(channelKey) {
        ...
        if (typeof this.channels[channelKey] != 'undefined' && typeof this.channels[channelKey]['channelDiscord'] != 'undefined') {
            ...
            return this.getDiscordChannelById(this.discord.channels.cache.find(ch => { return ch.id == this.channels[channelKey]['channelDiscord']; }));
        }
    ...
    getDiscordChannelById(channelId) {
        var discordChannel = this.discord.channels.cache.find(ch => { return ch.id == channelId; });
