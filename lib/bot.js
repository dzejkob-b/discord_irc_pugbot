...
class Bot {
    ...
    attachListeners() {
        if (this.discord != null) {
            ...
            this.discord.on('message', (message) => {
                if (message.channel && message.channel.id == this.channelDiscord) {

                    this.sendToIRC(message);

                    let partRef = null;

                    if (partRef = this.channelDisUsers.getUserId(message.author.id)) {

                        let baseText = this.parseText(message);

                        this.operRef.supplyCommand(partRef.getClone(), baseText, false);
        ...
        if (this.ircClient != null) {
            ...
            this.ircClient.on('message', (nick, to, text, message) => {
                if (to == this.nickname) {
                    ...
                    let partRef = new Participant({"author" : nick, "channel" : to});
            ...
            this.ircClient.on('join', (channelName, nick) => {
                ...
                let partRef = this.channelIrcUsers.joinUser(channel, nick);
            ...
            this.ircClient.on('part', (channelName, nick, reason) => {
                ...
                this.operRef.leaveEvent(partRef, "part");
            ...
            this.ircClient.on('kick', (channelName, nick, by, reason, message) => {
                ...
                let partRef = new Participant({"author" : nick, "channel" : channel});
                this.operRef.leaveEvent(partRef, "kick");

                this.sendExactToDiscord(channel, `*${nick}* was kicked from channel (${reason}) by *${by}*`);
            });
            ...
            this.ircClient.on('quit', (nick, reason, channels) => {
                ...
                channels.forEach((channelName) => {
                    ...
                    let partRef = new Participant({"author" : nick, "channel" : channel});
                    this.operRef.leaveEvent(partRef, "quit");
    ...
    sendToIRC(message) {
        if (!this.ircClient || !this.isReady()) return false;
        ...
        const ircChannel = this.channelIrc;
    ...
    getDiscordChannel() {
        var discordChannel = this.discord.channels.get(this.channelDiscord);

        if (!discordChannel) {
            logger.info(
                'Cannot get channel the bot isn\'t in: ',
                this.channelDiscord
            );
    ...
    sendToDiscord(author, channel, text) {
        if (!this.discord || !this.isReady()) return false;
