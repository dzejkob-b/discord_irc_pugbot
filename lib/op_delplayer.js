...
class OpDelPlayer {
    ...
    exec(channelKey) {
        ...
        if (!operRef.anyCats(channelKey)) {
            ...
            result = catRef.leaveParticipant(operRef.gameRef, cRef);

            if ((tt = cStk.pop()) && !isNaN(parseInt(tt)) && parseInt(tt) > 0) {
                catRef.addParticipantTimeout(operRef.gameRef, cRef, parseInt(tt));
                
            } else {
                catRef.addParticipantTimeout(operRef.gameRef, cRef);
            }
