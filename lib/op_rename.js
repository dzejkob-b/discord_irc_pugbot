...
class OpRename {
	...
	exec(channelKey) {
		...
		if (!operRef.anyCats(channelKey)) {
			...
			(removePartRef = catRef.getParticipantNickOrForceIndex(cStk.pop())) ==
				null ||
			(removePartIdx = catRef.findParticipant(removePartRef)) == -1
		) {
			partRef.noticeMessage(
				operRef,
				WordCo.cre()
				...
				(addPartRef) => {
					if (addPartRef == null) {
						...
						partRef.noticeMessage(
							operRef,
							WordCo.cre().text(
								"Cannot replace player when pug allready starting!"
							)
						);
					} else {
