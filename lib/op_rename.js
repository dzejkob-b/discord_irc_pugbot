...
class OpRename {
    ...
    exec(channelKey) {
        let operRef = this.parent, catRef, removePartRef, removePartIdx, cStk = this.parent.cStk, partRef = this.parent.partRef, privPartRef = this.parent.privPartRef;
        ...
        if (!operRef.anyCats(channelKey)) {
            ...
            operRef.getUser(cStk.pop(), (addPartRef) => {
                ...
                if (addPartRef == null) {
                ...
                } else if ((catRef.findParticipant(addPartRef)) != -1) {
                    ...
                    operRef.sendMsg(channelKey, WordCo.cre().text('Player ').texth(removePartRef.nick).text(' was replaced by ').texth(addPartRef.nick).text(' in ').texth(catRef.flag).text(' pug.'), privPartRef);
                }
