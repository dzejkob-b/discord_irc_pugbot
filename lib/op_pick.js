...
class OpPick {
    ...
    exec(channelKey) {
        ...
        if (operRef.logicState == 0 && (cStk.last() == 'p' || cStk.last() == 'promote')) {
            ...
            if ((catRef = operRef.getCatRef(channelKey, cStk.popMod(channelKey))) == null) {
                partRef.noticeMessage(operRef, WordCo.cre().text('No such pug ').texth(cStk.last()).text('!'));

            } else if (!catRef.isFull()) {
                // msg to all
                operRef.sendMsg(channelKey, WordCo.cre().text('Only ').texth(catRef.playerLimit - catRef.list.length, true).text(' needed for the ').texth(catRef.flag).text(' pug!').text(' Type ').texth('.j ' + catRef.flag).text(' to join.'), privPartRef);
            }
