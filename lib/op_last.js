...
class OpLast {
    ...
    exec(channelKey) {
        let operRef = this.parent, sf, hSteps = 0, useChan, hFlag = false, hCnt = false, hList = [], cStk = this.parent.cStk, partRef = this.parent.partRef, privPartRef = this.parent.privPartRef;

        if (operRef.currentCmd == 'lastall') {
            useChan = false;
        } else {
            useChan = channelKey;
        }
        ...
        if (operRef.currentCmd == 'last' || operRef.currentCmd == 'lastall') {
