...
class SumStats {
    ...
    eatIniProcess(path, callback) {
        ...
        if (fs.existsSync(path) && (data = fs.readFileSync(path, 'utf8')) != false) {
                ...
                if (line.length > 0) {
                    ...
                    if (line[0] == '[' && line[line.length - 1] == ']') {
                        ...
                        if (idxEq >= 0) {
                            ...
                            } else if (partKey == 'starttime' || partKey == 'last') {
                                if (partValue > useObj[partKey]) useObj[partKey] = partValue;
            ...
            var db = this.parent.getDb(), key, subKey;
                    ...
                    if (dt[key]["nick"]) {
                        ...
                        for (subKey in dt[key]) {
                            if (typeof dt[key][subKey] == 'object') {
                                ...
                                }, (function(tKey, tSubKey) { return function() {
    ...
    addUserBan(partRef, callback) {
        var db = this.parent.getDb(), final_row = null;
        ...
        db.serialize(() => {
            ...
            db.get(sql, dt, (function() { return function(err, row) {
        ...
        db.serialize(() => {
            ...
            if (typeof partRef == 'object') {
                if (partRef.type == 0) {
                    ...
                    pConds.push("( p.ircAuth = $ircAuth OR ((p.ircAuth IS NULL OR LENGTH(p.ircAuth) = 0) AND p.nick = $ircAuth) )");
                ...
                sql += "INNER JOIN stat st ON st.rowid = ps.stat_id AND st.flag = $flag ";
                sql += "WHERE (" + pConds.join(' OR ') + ") AND p.botIdent = $botIdent ";
