...
class DiscordUsers {
	...
	getUserDirect(nick) {
		...
		if (
			cPartRef != null &&
			(ref = this.botRef.discord.users.cache.find((us) => {
				return us.id == cPartRef.id;
			}))
		) {
			return ref;
		}
	...
	userQuerySearch(nick, callback) {
		this.botRef.discord.guilds.cache.forEach((gRef) => {
			if (gRef.id == this.botRef.sources.discord.guildId) {
				gRef.members
					.fetch({ query: nick, limit: 5 })
					.then((members) => {
						var inputList = [],
							member = null;
	...
	userIdSearch(id, callback) {
		this.botRef.discord.guilds.cache
			.map((g) => g)
			.forEach((gRef) => {
				if (gRef.id == this.botRef.sources.discord.guildId) {
					gRef.members
	...
	refreshKnownUsers(inputList) {
		...
		loopList.forEach((uRef) => {
			...
			const currentRefStatus = uRef?.presence?.status || "offline";
			if ((idx = this.getUserIdx(ref.id)) == -1) {
				...
				partRef.presence = currentRefStatus;
				...
				if (
					...
					if (!this.list[idx].offlineTime) {
						...
						this.list[idx].presence = currentRefStatus;
