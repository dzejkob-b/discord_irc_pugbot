...
class VoteOperator {
    ...
    static fromJSON(operRef, input) {
        var ref = new VoteOperator(operRef);
    ...
    findJoinedParticipant(partRef) {
        let cPartRef = null,
            cCatRef = null;

        if (this.operRef.logicState > 1) {
            cPartRef = this.operRef.gameCatRef.getParticipant(partRef);
            cCatRef = this.operRef.gameCatRef;
        } else {
            var idx = 0;

            while (cPartRef == null && idx < this.operRef.cats.length) {
                cCatRef = this.operRef.cats[idx];

                if (
                    (cPartRef = this.operRef.cats[idx].getParticipant(
                        partRef
                    )) == null
                ) {
                    idx++;
    ...
    getParticipantForVoteNickOrForceIndex(input) {
        let cPartRef = null,
            cCatRef = null;

        if (this.operRef.logicState > 1) {
            cPartRef = this.operRef.gameCatRef.getParticipantNickOrForceIndex(
                input
            );
            cCatRef = this.operRef.gameCatRef;
        } else {
            ...
            while (cPartRef == null && idx < this.operRef.cats.length) {
                ...
                if (
                    (cPartRef = this.operRef.cats[
                        idx
                    ].getParticipantNickOrForceIndex(input)) == null
                ) {
                    idx++;
                }
            }
    ...
    getVote(partRef, trgPartRef) {
        ...
        while (idx < this.votes.length) {
            if (
                this.votes[idx]['partRef'].compareEqual(partRef) &&
                this.votes[idx]['trgPartRef'].compareEqual(trgPartRef)
            ) {
    ...
    addVote(operRef, catRef, partRef, trgPartRef, value) {
        if (this.getVote(partRef, trgPartRef)) {
            return -1;
        } else {
            let idxs = this.getVotes(partRef),
                totalVotes = 0;

            if (operRef.logicState > 0 && idxs.length >= catRef.possibleVotes) {
                return -3;
            }
    ...
    getVoteSumarize(exclude, minVotes, excludeNocapt) {
        ...
        while (idx < this.votes.length) {
            if (
                (!exclude ||
                    !this.votes[idx]['trgPartRef'].isInArray(exclude)) &&
                ...
                while (!brk && pos < result.length) {
                    if (
                        result[pos]['trgPartRef'].compareEqual(
                            this.votes[idx]['trgPartRef']
                        )
                    )
                        brk = true;
                    else pos++;
            ...
            while (idx < result.length) {
                ...
                wRef.texth(
                    '[' +
                        result[idx]['votes'] +
                        'v' +
                        (result[idx]['trgPartRef'].isNocapt()
                            ? ', nocapt'
                            : '') +
                        ']'
                );
