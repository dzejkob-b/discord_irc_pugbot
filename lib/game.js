...
class Game {
    ...
    constructor(channelKey, catRef, teamCount) {
        ...
        if (catRef) {
            ...
            this.restCat = new Catalog(catRef.channelKey, catRef.flag, catRef.playerLimit, catRef.teamCount, catRef, catRef.getExtraSettings());
    ...
    doPickCaptains(operRef, voteRef, callback) {
        ...
        captainPos = new Catalog(this.restCat.channelKey, this.restCat.flag, this.restCat.playerLimit, this.restCat.teamCount, null, this.restCat.getExtraSettings());
        ...
        captainAssigned = new Catalog(this.restCat.channelKey, this.restCat.flag, this.restCat.playerLimit, this.restCat.teamCount, null, this.restCat.getExtraSettings());
    ...
    doPickCaptainsFinal(operRef, captainPos, sortedParts, callback) {
        ...
        var idx = 0, lastAvg = false, useParts = [[], []];
        ...
        */
        ...
        if (sortedParts && sortedParts.length > 0) {
            sortedParts.forEach((c) => {
                ...
                var idx = c['partRef'].tPugs && c['partRef'].avgPick && c['partRef'].tPugs >= 10 && c['partRef'].avgPick <= 6 ? 0 : 1;
                ...
                if (upIdx != -1) {
                    ...
                    if (lastAvg === false) {
                        ...
                        while (avgIdx < useParts[upIdx].length) {
                            if (minDiffIdx == -1 || Math.abs(lastAvg - useParts[upIdx][avgIdx]['partRef'].avgPick) < minDiff) {
                                minDiffIdx = avgIdx;
                                minDiff = Math.abs(lastAvg - useParts[upIdx][avgIdx]['partRef'].avgPick);
                            }
                    ...
                    this.teams[idx].setCaptParticipant(useParts[upIdx][avgIdx]['partRef'], 'avgpick');
