...
class Game {
	...
	doPickCaptainsFinal(operRef, captainAssignedParts, captainPos, sortedParts, callback) {
		var idx = 0,
			lastAvg = false,
			useParts = [[], [], []];
        ...
        */
		...
		if (sortedParts && sortedParts.length > 0) {
			sortedParts.forEach((c) => {
				...
				if (c["partRef"].tPugs && c["partRef"].avgPick && c["partRef"].tPugs >= 10 && c["partRef"].avgPick <= 6) {
					idx = 0;
				...
				if (upIdx != -1) {
					...
					if (upIdx == 2) {
						...
						while (avgIdx < useParts[upIdx].length) {
							if (
								minDiffIdx == -1 ||
								Math.abs(lastAvg - useParts[upIdx][avgIdx]["partRef"].avgPick) < minDiff
							) {
								minDiffIdx = avgIdx;
								minDiff = Math.abs(
