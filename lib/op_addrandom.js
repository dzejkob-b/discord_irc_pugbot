    ...
    exec(channelKey) {
        ...
        if (operRef.logicState != 0) {
            partRef.noticeMessage(
                operRef,
                WordCo.cre().text('Cannot add random player - pug starting!')
            );
            ...
            if (operRef.currentCmd == 'addrandom') {
                numCnt = !isNaN(parseInt(cStk.pop()))
                    ? parseInt(cStk.last())
                    : 1;
            ...
            if (
                new RegExp('^[a-zA-Z]{1}[a-zA-Z0-9]{0,19}$').test(uTag) == false
            )
                uTag = '';
            ...
            if (operRef.currentCmd == 'addcustom') {
                ...
                if (
                    (result = catRef.joinParticipant(cRef)) == 0 ||
                    result == 1
                ) {
                    partRef.noticeMessage(
                        operRef,
                ...
                do {
                    ...
                    if (
                        ...
                        partRef.noticeMessage(
                            operRef,
                            WordCo.cre()
                                .text('Cannot add random player ')
                                .texth(cRef.nick)
                                .text(' to ')
                                .texth(catRef.flag)
                                .text(' pug. Result error: ')
                                .texth(result)
                        );
                    }
