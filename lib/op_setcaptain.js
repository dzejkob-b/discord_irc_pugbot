...
class OpSetCaptain {
    ...
    exec(channelKey) {
        ...
        if (operRef.logicState == 1 || operRef.logicState == 2) {
            ...
            if (
                (cPartRef = operRef.gameCatRef.getParticipantNickOrForceIndex(
                    cStk.pop()
                )) == null
            ) {
                ...
                if (teamColor) {
                    if (
                        (teamRef = operRef.gameRef.getTeamByColor(teamColor)) ==
                        null
                    ) {
                        partRef.noticeMessage(
                            operRef,
                    ...
                    if (
                        (teamRef = operRef.gameRef.getFirstNocaptTeam()) == null
                    ) {
                        partRef.noticeMessage(
                            operRef,
                            WordCo.cre()
                                .text(
                                    'All captains is set. Please use command with color '
                                )
                ...
                if (teamRef != null) {
                    ...
                    operRef.gameRef.setCaptainToTeam(
                        teamRef,
                        cPartRef,
                        'enforced'
                    );
                    ...
                    if (
                        befNocaptCount == 0 &&
                        operRef.gameRef.getNonCaptainCount() > 0
                    ) {
