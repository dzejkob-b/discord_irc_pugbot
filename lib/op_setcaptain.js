...
class OpSetCaptain {
    ...
    exec(channelKey) {
        let operRef = this.parent, acRef = operRef.getAction(channelKey), cRef, teamRef, teamColor, wRef, cStk = this.parent.cStk, partRef = this.parent.partRef, privPartRef = this.parent.privPartRef;

        if (acRef.logicState == 1 || acRef.logicState == 2) {
            var cPartRef;

            if ((cPartRef = acRef.gameCatRef.getParticipantNickOrForceIndex(cStk.pop())) == null) {
                partRef.noticeMessage(operRef, WordCo.cre().text('No user ').texth(cStk.last()).text(' in the ').texth(acRef.gameRef.restCat.flag).text(' pug!'));
