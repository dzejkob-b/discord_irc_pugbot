    ...
    getStatsMultiple(method, flag, partsList, callback) {
        if (method['type'] == 'sumarize') {
            ...
            db.serialize(() => {
                ...
                sql += "SELECT ps.*, s.flag, s.starttime, COUNT(p2.rowid) AS 'pickPlayers', p.*, p.rowid AS 'player_id', ";
                ...
                db.all(sql, dt, function(err, rows) {
                    ...
                    if (rows) {
                        rows.forEach((row) => {
                            ...
                            if (method['type'] != 'pastpicks' || method['type'] == 'unlimited' || pdt[pid]["st_recs"] <= method['value']) {
                                ...
                                if (!pdt[pid]['min_starttime'] || row['starttime'] < pdt[pid]['min_starttime']) {
            ...
            db.close(() => {
                ...
                if (st_rows) {
                    ...
                    db.serialize(() => {
                        ...
                        sql += "ORDER BY ps.teamIndex ASC, ps.isCaptain DESC, ps.pickIndex ASC, ps.ext_stat_id ASC";
