...
class Stats {
            ...
            db.serialize(() => {
                ...
                while (idx < parts.length) {
                    ...
                    if (c['player_id']) {
                        ...
                        if (c['partRef'].type == 0) {
                            db.run(
                                'UPDATE player SET discordId = $discordId WHERE rowid = $player_id',
                                {
    ...
    getUser(partRef) {
        ...
        db.serialize(() => {
            ...
            sql += 'LEFT JOIN player_stat ps ON ps.player_id = p.rowid '
            sql +=
                'LEFT JOIN stat st ON st.rowid = ps.stat_id AND st.flag = $flag '
            sql +=
                'WHERE (' +
                pConds.join(' OR ') +
                ') AND p.botIdent = $botIdent '
            ...
            db.close(() => {
                if (st_rows) {
                    ...
                    db.serialize(() => {
                        ...
                        sql +=
                            'ORDER BY ps.teamIndex ASC, ps.pickIndex ASC, ps.ext_stat_id ASC'
