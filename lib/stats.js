import SumStats from "./sum_stats.js";
...
class Stats {
	...
	createStructure() {
		...
		db.serialize(() => {
			db.run(
				"CREATE TABLE IF NOT EXISTS player (botIdent TEXT, nick TEXT, ircAuth TEXT, discordId TEXT, bans INTEGER)"
			);

			db.run(
				"CREATE TABLE IF NOT EXISTS stat (flag TEXT, captained INTEGER, starttime INTEGER, totalpugs INTEGER, last INTEGER, picks INTEGER, totalpicks INTEGER)"
			);

			db.run(
				"CREATE TABLE IF NOT EXISTS player_stat (player_id INTEGER, stat_id INTEGER)"
			);

			db.run(
				"CREATE TABLE IF NOT EXISTS ext_stat (channelKey TEXT, flag TEXT, isQuick INTEGER, starttime INTEGER)"
			);

			db.run(
				"CREATE TABLE IF NOT EXISTS ext_player_stat (player_id INTEGER, ext_stat_id INTEGER, tag TEXT, pickIndex INTEGER, isCaptain INTEGER, votes INTEGER, teamIndex INTEGER, teamName TEXT)"
			);

			try {
				db.run(
					"ALTER TABLE stat ADD COLUMN channelKey TEXT",
					function (err, row) {
						return false;
					}
				);
	...
	saveGameToStats(gameRef, voteRef) {
		...
		db.serialize(() => {
			...
			while (idx < parts.length) {
				...
				sql +=
					"SELECT p.rowid AS 'player_id', st.rowid AS 'stat_id' FROM player p ";
				sql += "LEFT JOIN player_stat ps ON ps.player_id = p.rowid ";
				sql +=
					"LEFT JOIN stat st ON st.rowid = ps.stat_id AND st.flag = $flag AND st.channelKey = $channelKey ";
				sql +=
					"WHERE (" + pConds.join(" OR ") + ") AND p.botIdent = $botIdent ";
				sql +=
			...
			db.serialize(() => {
				...
				while (idx < parts.length) {
					...
					if (c["player_id"]) {
						...
						if (c["partRef"].type == 0) {
							db.run(
								"UPDATE player SET discordId = $discordId, nick = $nick WHERE rowid = $player_id",
								(dt = {
							...
							db.run(
								"UPDATE player SET ircAuth = $ircAuth WHERE rowid = $player_id",
								(dt = {
						...
						switch (c["partRef"].type) {
							case 0:
								db.run(
									"INSERT INTO player (botIdent, nick, discordId, bans) VALUES ($botIdent, $name, $discordId, $bans)",
									{
							...
							default:
								if (c["partRef"].whois && c["partRef"].whois["account"]) {
									db.run(
										"INSERT INTO player (botIdent, nick, ircAuth, bans) VALUES ($botIdent, $name, $ircAuth, $bans)",
										{
											$botIdent: this.operRef.ident,
											$name: c["partRef"].nick,
											$ircAuth: c["partRef"].whois["account"],
				...
				db.serialize(() => {
					...
					while (idx < parts.length) {
						if (parts[idx]["stat_id"]) {
							db.run(
								"UPDATE stat SET captained = captained + $captained, totalpugs = totalpugs + $totalpugs, last = $last, picks = picks + $picks, totalpicks = totalpicks + $totalpicks WHERE rowid = $stat_id",
								{
									$stat_id: parts[idx]["stat_id"],
									$captained: parts[idx]["captain"] ? 1 : 0,
									$totalpugs: 1,
									$last: new Date().getTime() / 1000,
									$picks: parts[idx]["captain"]
										? 0
										: parts[idx]["pickIndex"] + 1,
									$totalpicks: parts[idx]["captain"] ? 0 : partsTotalPicks,
								}
							);
						} else {
							db.run(
								"INSERT INTO stat (channelKey, flag, captained, starttime, totalpugs, last, picks, totalpicks) VALUES ($channelKey, $flag, $captained, $starttime, $totalpugs, $last, $picks, $totalpicks)",
								{
				...
				db.close(() => {
					...
					db.close(() => {
						...
						db.serialize(() => {
							...
							db.run(
								"INSERT INTO ext_stat (channelKey, flag, isQuick, starttime) VALUES ($channelKey, $flag, $isQuick, $starttime)",
								{
									$channelKey: gameRef.channelKey,
									$flag: gameRef.restCat.flag,
								...
								function (err, row) {
									...
									while (idx < parts.length) {
										db.run(
											"INSERT INTO ext_player_stat (player_id, ext_stat_id, tag, pickIndex, isCaptain, votes, teamIndex, teamName) VALUES ($player_id, $ext_stat_id, $tag, $pickIndex, $isCaptain, $votes, $teamIndex, $teamName)",
											{
												$player_id: parts[idx]["player_id"],
												...
												$votes:
													voteRef != null
														? voteRef.getTotalVotes(parts[idx]["partRef"])
														: 0,
												$teamIndex: parts[idx]["teamIndex"],
												$teamName:
													parts[idx]["teamRef"] == null
														? ""
														: parts[idx]["teamRef"].colorName,
											}
										);
	...
	getStatsMultiple(method, flag, partsList, callback) {
		if (method["type"] == "sumarize") {
			...
			db.serialize(() => {
				...
				sql +=
					"SELECT ps.*, s.flag, s.starttime, COUNT(p2.rowid) AS 'pickPlayers', p.*, p.rowid AS 'player_id', ";
				sql +=
					"julianday('now') - julianday(s.starttime, 'unixepoch') AS 'past_days', datetime(s.starttime, 'unixepoch') AS 'date' ";
				sql += "FROM player p ";
				sql += "INNER JOIN ext_player_stat ps ON ps.player_id = p.rowid ";
				sql += "INNER JOIN ext_stat s ON ps.ext_stat_id = s.rowid ";
				...
				sql +=
					"INNER JOIN ext_player_stat p2 ON p2.ext_stat_id = s.rowid AND p2.isCaptain = 0 ";
				sql +=
					"WHERE (" + pConds.join(" OR ") + ") AND p.botIdent = $botIdent ";

				if (method["type"] == "pastdays") {
					sql +=
						" AND (julianday('now') - julianday(s.starttime, 'unixepoch')) <= $pastdays ";
					dt["$pastdays"] = method["value"];
				}

				if (!flag) {
					sql += " AND s.isQuick = 0 ";
					sql += " GROUP BY s.flag, p.rowid, s.rowid ";
					sql += " ORDER BY s.flag ASC, p.rowid ASC, s.rowid DESC ";
				} else {
				...
				db.all(sql, dt, function (err, rows) {
					...
					if (rows) {
						rows.forEach((row) => {
							var pid = row["flag"] + "~" + parseInt(row["player_id"]);
							...
							if (
								method["type"] != "pastpicks" ||
								method["type"] == "unlimited" ||
								pdt[pid]["st_recs"] <= method["value"]
							) {
								pdt[pid]["st_recs"]++;
								pdt[pid]["st_totalpugs"]++;

								if (row["starttime"] > pdt[pid]["starttime"]) {
									pdt[pid]["starttime"] = row["starttime"];
									pdt[pid]["date"] = row["date"];
								}

								if (
									!pdt[pid]["min_starttime"] ||
									row["starttime"] < pdt[pid]["min_starttime"]
								) {
									pdt[pid]["min_starttime"] = row["starttime"];
									pdt[pid]["min_date"] = row["date"];
								}
	...
	getGameStat(channelKey, flag, player_id, index, callback) {
		...
		db.serialize(() => {
			...
			if (player_id && (tt = parseInt(player_id)) && !isNaN(player_id)) {
				sql +=
					"INNER JOIN ext_player_stat x ON x.ext_stat_id = st.rowid AND x.player_id = " +
					player_id +
					" ";
			}

			sql +=
				(pConds.length > 0 ? "WHERE " + pConds.join(" AND ") + " " : "") +
				" ) AS 'count' ";
			...
			db.close(() => {
				if (st_rows) {
					...
					db.serialize(() => {
						...
						sql +=
							"ORDER BY ps.teamIndex ASC, ps.isCaptain DESC, ps.pickIndex ASC, ps.ext_stat_id ASC";
	...
	getTrendStats(catRef, days, callback) {
		...
		db.serialize(() => {
			...
			sql +=
				"SELECT *, s.rowid AS 'ext_stat_id', datetime(s.starttime, 'unixepoch') AS 'date', ";
			...
			sql += "FROM ext_stat s ";
			sql +=
				"WHERE datetime(s.starttime, 'unixepoch') >= datetime($startDate) AND datetime(s.starttime, 'unixepoch') <= datetime($endDate) ";
				...
				if (rows && rows.length > 0) {
					rows.forEach((row) => {
						while (idx < result.length) {
							...
							if (
								c_year == parseInt(row["year"]) &&
								c_month == parseInt(row["month"]) &&
								c_day == parseInt(row["day"])
							) {
