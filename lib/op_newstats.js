...
class OpNewStats {
    ...
    exec(channelKey) {
        let operRef = this.parent, nick, lastPop = false, catRef, cStk = this.parent.cStk, partRef = this.parent.partRef;
        ...
        if (operRef.currentCmd == 'mystats') {
            ...
            operRef.getUser(nick, (cPartRef) => {

                if (cPartRef == null) {
                    partRef.noticeMessage(operRef, WordCo.cre().text('No player entry for ').texth(nick).text('!'));
    ...
    displayStats(channelKey, cPartRef, catRef) {
        var operRef = this.parent, pList = [], partRef = this.parent.partRef, privPartRef = this.parent.privPartRef;

        pList.push(cPartRef);

        operRef.statsRef.getStatsMultiple(operRef.statsRef.parseAvgStatMethod('unlimited'), catRef ? catRef.flag : null, pList, (rows) => {
            ...
            rows.forEach((row) => {
                ...
                if (tPugs > 0) {
                    ...
                    tAvgTotPick = Math.round((row['totalpicks'] / tPugs) * 100.0) / 100.0;
                }

                msgs.push(WordCo.cre());

                var cTm = (new Date()).getTime() / 1000;
                var dailyStat = Math.round((row['totalpugs'] / ((cTm - row['min_starttime']) / 86400)) * 100.0) / 100.0;
                ...
                wRef.text('Pick: ');
                wRef.texth('[').text(tAvgPick).texth('/').text(tAvgTotPick).texth(']');
                ...
                if (lastDiff > 0) {
                    wRef.texth('[').text(secsAgoFormat(lastDiff) + ' ago').texth(']');
                } else {
