...
class Operator {
    constructor(botRef, options) {
        ...
        this.cmds = {
            ...
            'list' : {
                'alt' : ['ls', 'liast'], 'auth' : 0, 'inst' : new OpList(this), 'banable' : false,
            ...
            'pick' : {
                'alt' : ['p', 'promote'], 'auth' : 0, 'inst' : new OpPick(this), 'banable' : false,
                'info' : 'Pick player to your team (May use only captain). Usage: !pick playername|playernumber'
            },
            'last' : {
                'alt' : ['lastt', 'lasttt', 'lastttt'], 'auth' : 0, 'inst' : new OpLast(this), 'banable' : false,
    ...
    getCatsInChannel(channelKey) {
        ...
        this.cats.forEach((catRef) => {
            if (catRef.channelKey == channelKey) {
                chCats.push(catRef);
            }
        });
    ...
    getCatRef(channelKey, flag) {
        ...
        if (flag) {
            ...
            while (idx < chCats.length) {
                if (chCats[idx].channelKey == channelKey && chCats[idx].flag == flag) {
                    return chCats[idx];
    ...
    getCatRefByFlag(channelKey, flag) {
        ...
        while (idx < this.cats.length) {
            if (this.cats[idx].channelKey == channelKey && this.cats[idx].flag == flag) {
    ...
    supplyCommandFn(channelKey, partRef, cStk, privMsg) {
        ...
        if (toFnd) {
            if (this.textCommands[channelKey + '::' + toFnd] && Array.isArray(this.textCommands[channelKey + '::' + toFnd])) {
                this.textCommands[channelKey + '::' + toFnd].forEach((msg) => {

                    if (msg == "::DELETE") {
                        return false;

                    } else {
                        this.sendMsg(channelKey, WordCo.cre().text(msg.replace('::NEWLINE ', '\n').replace('::NEWLINE', '\n')), privPartRef);
                    }
        ...
        if (toFnd) {
            if (this.textCommands[toFnd] && Array.isArray(this.textCommands[toFnd])) {
                this.textCommands[toFnd].forEach((msg) => {
    ...
    sendMsg(channelKey, text, privPartRef = null) {
        if (privPartRef != null) {
            ...
            this.botRef.sendExactToIRC(channelKey, text.getIrc());
            ...
            this.botRef.sendExactToIRC(channelKey, text);
    ...
    sendMsgArray(channelKey, list, idx, privPartRef = null) {
        if (privPartRef != null) {
            privPartRef.noticeMessage(this, list[idx].getIrc());

        } else {
            
            this.botRef.sendExactToIRC(channelKey, list[idx].getIrc());
        ...
        if (idx + 1 < list.length) {
            setTimeout(() => {
                ...
                this.sendMsgArray(channelKey, list, idx + 1, privPartRef);
