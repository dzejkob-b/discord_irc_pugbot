...
class Operator {
    constructor(botRef, options) {
        ...
        this.cmds = {
            ...
            'rename' : { 'alt' : [ 'replaceplayer' ], 'auth' : 10, 'info' : 'Replace player in pug by someone else. Usage: !rename [pug] player newPlayer' },
    ...
    getCmdAuth(cmd, flag = 'auth') {
        ...
        if (flag == 'auth') {
            return this.textCommands[cmd] ? 0 : 10; // text commands are permitted
        } else {
            return false;
        }
    }
    ...
    supplyCommandFn(partRef, cStk, privMsg) {
        ...
        let cRef, catRef, wRef, cmdName, reason, privPartRef = privMsg ? partRef : null;
        let result;
        let teamRef;
        ...
        switch (currentCmd) {
            ...
            case 'vote' :
                if (this.logicState == 1 && this.gameRef != null) {

                    if (this.voteRef == null) {
                        this.voteRef = new VoteOperator(this.gameRef.restCat, this.gameRef.teams.length);
                    }

                    if (this.voteRef.catRef.findParticipant(partRef) == -1) {
                        partRef.noticeMessage(this, WordCo.cre().text('You cannot vote you are not joined in the ').texth(this.voteRef.catRef.flag).text('pug!'));

                    } else {
                        let votePartRef = this.voteRef.catRef.getParticipantNickOrForceIndex(cStk.pop());

                        if (votePartRef == null) {
                            partRef.noticeMessage(this, WordCo.cre().text('No such user ').texth(cStk.last()).text(' in the ').texth(this.voteRef.catRef.flag).text(' pug!'));

                        } else {
                            let voteResult = this.voteRef.addVote(partRef, votePartRef, 1);

                            if (voteResult >= 0) {
                                this.captainTick = 3;

                                partRef.noticeMessage(this, WordCo.cre().text('You voted for ').texth(votePartRef.nick).text('.'));
                                ...
                                partRef.noticeMessage(this, WordCo.cre().text('You cannot vote for yourself!'));
            ...
            case 'mention' :
                this.getUser(cStk.pop(), (cPartRef) => {
                    ...
                    if (cPartRef == null) {
                        ...
                        cPartRef.noticeMessage(this, WordCo.cre().textNick(cPartRef.nick, false, cPartRef.getDiscordId()).text(', you was mentioned by ').texth(partRef.nick).text(': ' + reason));

                        this.sendMsg(false, WordCo.cre().textNick(cPartRef.nick, false, cPartRef.getDiscordId()).text(', ' + reason), privPartRef);

                    } else {
                        cPartRef.noticeMessage(this, WordCo.cre().textNick(cPartRef.nick, false, cPartRef.getDiscordId()).text(', you was mentioned by ').texth(partRef.nick));
