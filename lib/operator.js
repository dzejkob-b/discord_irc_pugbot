...
class Operator {
        ...
        if (this.botRef.isReady()) {
            ...
            this.cats.forEach((catRef) => {
                ...
                if (toKick.length > 0) {
                    toKick.forEach((partRef) => {
                        ...
                        if (isCaptainIdle) {
                            sendMsg = catRef.leaveParticipantCooldown(this.gameRef, partRef, isCaptainIdle) == 0;
            ...
            if (this.logicState == 0) {
                ...
                if (this.gameRef.teams.length == 0) {
                    ...
                    this.gameRef.getAllParticipants().forEach((cPartRef) => {
                        const removedPugs = this.removePlayerFromAllOtherCatalogs(this.gameRef.restCat, cPartRef); // should probably be the other way around.
                        ...
                        cPartRef.personalMessage(this, message, true);
                ...
                if (this.gameRef.captainIdle && this.gameRef.captainIdlePart) {
                    ...
                    wRef = WordCo.cre().texth(this.gameRef.captainIdlePart.nick).text(` wake up! You have ${(this.gameRef.captainIdleHeadsUpInSecs / 1000)} seconds to pick a player. Type '.here' for extra time.`);

                    this.gameRef.captainIdlePart.noticeMessage(this, wRef);
                    this.sendMsg(gameCatRef.channelKey, wRef);
    ...
    removePlayerFromAllOtherCatalogs(restCat, cPartRef) {
        const signedPugs = this.getCatsInChannel(false).filter(cat => cat.flag != restCat.flag);
        signedPugs.forEach(cat => {
            cat.leaveParticipant(cPartRef);
        })
        return signedPugs;
    }
