...
class Operator {
    constructor(botRef, options) {
        ...
        this.cmds = {
            ...
            'avgpick' : {
                'alt' : [], 'auth' : 0, 'inst' : new OpAvgPick(this), 'banable' : false,
    ...
    startSelectCaptains(catRef, startSeconds) {
        ...
        this.getCatsInChannel(catRef.channelKey).forEach((rCatRef) => {
            if (rCatRef.flag != catRef.flag) {
                rCatRef.leaveParticipantList(this.gameRef, catRef.list, true);
            }
        });
        
        // create game reference

        this.gameRef = new Game(catRef.channelKey, catRef, catRef.teamCount, catRef.extraSettings);
        ...
        if (this.botRef.isReady()) {
            ...
            this.cats.forEach((catRef) => {
                ...
                if (toKick.length > 0) {
                    toKick.forEach((partRef) => {
                        const isCaptainIdle = this.gameRef.getTeamByCaptain(toKick) != null; // todo: need to check it.
                        if (catRef.leaveParticipant(this.gameRef, partRef, isCaptainIdle) == 0) {
            ...
            if (flagsRem.length > 0) {
                flagsRem.forEach((c) => {
                    ...
                    // msg to all
                    this.sendMsg(c["channelKey"], WordCo.cre().text('The ').texth(c["flag"]).text(' pug was removed for inactivity.'));
            ...
            if (this.logicState == 0) {
                ...
                if (this.gameRef.teams.length == 0) {
                    ...
                    wRef = WordCo.cre();
                    wRef.text('Picking random captains in ').texth(this.captainPickSeconds).text(' seconds. Players tagged nocapt will be avoided. Type ');
                    wRef.texth('!captain').text(' to become a captain. Use ');
                ...
                if (this.gameRef.captainIdle) {
                    this.sendMsg(gameCatRef.channelKey, WordCo.cre().text('The ').texth(gameCatRef.flag).text(' pug stopped because ').texth(this.gameRef.captainIdlePart.nick).text(' was idle.'));
                    this.gameCatRef.leaveParticipant(this.gameRef, this.gameRef.captainIdlePart, false, false, true);
                    this.gameRef.clearCaptainIdle();
                    this.logicState = 0;
                } else if (this.gameRef.captainIdleHeadsUpReached && this.gameRef.captainIdlePart) { // because of 'this.gameRef.captainIdlePart' in condition, when bot died in picking it won't give headsup.. should be improved.
                    this.gameRef.captainIdleHeadsUpReached = false; 
                    this.sendMsg(gameCatRef.channelKey, WordCo.cre().texth(this.gameRef.captainIdlePart.nick).text(` wake up! You have ${(this.gameRef.captainIdleHeadsUpInSecs /1000)} seconds to pick a player. Type '.here' for extra time.`));
                }
            }
    ...
    getTextCommand(channelKey, cmdKey, getAll = false, callback = false, callbackData = false) {

        let mList = false, wRef, idx, chKeys = this.botRef.getChannelKeys(), useChKeys, lst = [];
        ...
        for (let ck of lst) {
            if (this.textCommands[ck['toFnd']] && Array.isArray(this.textCommands[ck['toFnd']])) {
                ...
                if (this.textCommands[ck['toFnd']][0] == "::DELETE") {
                    ...
                    chKeys.forEach((c) => {
                        if (this.textCommands[c + '::' + cmdKey] && Array.isArray(this.textCommands[c + '::' + cmdKey])) {
