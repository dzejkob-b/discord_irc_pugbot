...
class Operator {
    constructor(botRef, options) {
        ...
        this.limitNoCaptTag = options.limitNoCaptTag || 0; // 0 - NoLimit
        ...
        this.cmds = {
            ...
            command: {
                ...
                banable: false,
                info:
                    'Shows commands or command details. Usage: !command [command]',
            },
            join: {
                alt: ['a', 'j', 'add', 'join'],
                auth: 0,
            ...
            delplayer: {
                ...
                banable: false,
                info:
                    'Remove player from pug. Usage: !delplayer [pug] playername',
            },
            rename: {
            ...
            welcome: {
                ...
                banable: false,
                info:
                    'Send welcome message to user. Usage: !welcome playername',
            },
            captain: {
                auth: 0,
            ...
            delhistory: {
                ...
                banable: false,
                info:
                    'Delete specified history entry. Usage: !delhistory number',
            },
            createpug: {
                auth: 10,
                ...
                banable: false,
                info:
                    'Create pug. Usage: !createpug pugName playersCount [teamsCount]',
            },
            quickpug: {
                auth: 0,
                inst: new OpCreatePug(this),
            ...
            stats: {
                ...
                banable: false,
                info:
                    'Display pug statistics of specific player. Usage: !stats [pug] playername',
            },
            mystats: {
                auth: 0,
            ...
            delgrant: {
                ...
                banable: false,
                info:
                    'Remove user from grant table. Usage: !delgrant playername',
            },
            grantlist: {
                auth: 10,
    ...
    toJSON() {
        var result = {
            ...
            gameCatRef:
                this.gameCatRef != null ? this.gameCatRef.toJSON() : null,
    ...
    fromJSON(input) {
        ...
        this.gameRef = input['gameRef']
            ? Game.fromJSON(input['gameRef'])
            : null;
        this.gameCatRef = input['gameCatRef']
    ...
    loadState() {
        ...
        if (
            fs.existsSync('persistent.json') &&
            (data = fs.readFileSync('persistent.json', 'utf8')) != false &&
            (dt = JSON.parse(data))
        ) {
            this.fromJSON(dt);
        }
        ...
        if (
            fs.existsSync('config_live.json') &&
            (data = fs.readFileSync('config_live.json', 'utf8')) != false &&
            (dt = JSON.parse(data))
        ) {
    ...
    userActivityEvent(partRef) {
        this.cats.forEach((catRef) => {
            let fndPartRef = catRef.getParticipantNickOrForceIndex(
                partRef.nick
            );
    ...
    supplyCommandFn(channelKey, partRef, cStk, privMsg) {
        ...
        if (
            ...
            if (bb['duration'] == 0) {
                wRef.text('You are premanently banned by ')
                    .texth(bb['by'].nick)
                    .text(' for: ')
                    .texth(bb['reason']);
            } else {
                wRef.text('You are banned by ')
                    .texth(bb['by'].nick)
                    .text(' for: ')
                    .texth(bb['reason']);
    ...
    startSelectCaptains(catRef, startSeconds) {
        ...
        this.gameRef = new Game(
            catRef.channelKey,
            catRef,
            catRef.teamCount,
            catRef.extraSettings
        );
        ...
        if (this.botRef.isReady()) {
            ...
            this.cats.forEach((catRef) => {
                ...
                if (toKick.length > 0) {
                    toKick.forEach((partRef) => {
                        const isCaptainIdle =
                            this.gameRef.getTeamByCaptain(toKick) != null; // todo: need to check it.
                        var sendMsg = false;
            ...
            if (toUnset.length > 0) {
                toUnset.forEach((authKey) => {
                    ...
                    wRef.text('Player ')
                        .texth(
                            c['partRef'] == null ? authKey : c['partRef'].nick
                        )
                        .text(' was unbaned.');
            ...
            if (this.logicState == 0) {
                ...
                if (this.gameRef.teams.length == 0) {
                    ...
                    tmList.push(
                        WordCo.cre().text(
                            'Pug is without teams so go server now please!'
                        )
                    );
                    ...
                    this.gameRef.getAllParticipants().forEach((cPartRef) => {
                        cPartRef.personalMessage(
                            ...
                            WordCo.cre()
                                ...
                                .texth(this.gameRef.restCat.flag)
                                .text(
                                    ' pug has been filled! Go server now please!'
                                ),
                            true
                        );
                    });
                    ...
                    this.gameRef.getAllParticipants().forEach((cPartRef) => {
                        cPartRef.personalMessage(
                            ...
                            WordCo.cre()
                                ...
                                .texth(this.gameRef.restCat.flag)
                                .text(
                                    ' pug has been filled! Please be prepared.'
                                ),
                            true
                        );
                    });
                    ...
                    wRef = WordCo.cre();
                    wRef.text('Picking random captains in ')
                        .texth(this.captainPickSeconds)
                        .text(
                            ' seconds. Players tagged nocapt will be avoided. Type '
                        );
                    wRef.texth('!captain').text(' to become a captain.');

                    if (this.gameRef.restCat.possibleVotes > 0) {
                        wRef.text(' Use ');
                        wRef.texth('!vote [player]').text(
                    ...
                    this.gameRef.doPickCaptains(this, this.voteRef, () => {
                        var teamRef,
                            tmList = [];
                        ...
                        if (!self.captainForcePicked) {
                            ...
                            while (idx < self.gameRef.teams.length) {
                                ...
                                if (teamRef.captPartRef.captainSetMethod) {
                                    ...
                                    switch (
                                        teamRef.captPartRef.captainSetMethod
                                    ) {
                                        case 'enforced':
                                    ...
                                    if (teamRef.captPartRef.avgTotPick) {
                                        wRef.text(' ')
                                            ...
                                            .text(teamRef.captPartRef.avgPick)
                                            .texth('/')
                                            .text(
                                                teamRef.captPartRef.avgTotPick
                                            )
                                            .texth(']')
                                            .text(' ');
                                    } else if (cptSetFlag) {
                                ...
                                teamRef.captPartRef.personalMessage(self, wRef);
                        ...
                        wRef = self.gameRef.restCat.addStatusReadable(
                            wRef,
                            true,
                            self.voteRef,
                            chk,
                        ...
                        if (tmList.length > 0) {
                            self.sendMsgArrayPrep(chk, tmList);
                        }
                    });
                }

                if (tmList.length > 0) {
                    this.sendMsgArrayPrep(chk, tmList);
                }
    ...
    isCommandExist(channelKey, cmdKey, allowDelete = true) {
        ...
        for (let ck of [channelKey + '::' + cmdKey, cmdKey]) {
            if (this.textCommands[ck] && Array.isArray(this.textCommands[ck])) {
                if (allowDelete && this.textCommands[ck][0] == '::DELETE') {
                    break;
                } else {
                    return true;
                }
    ...
    getTextCommand(
        ...
        for (let ck of lst) {
            if (
                this.textCommands[ck['toFnd']] &&
                Array.isArray(this.textCommands[ck['toFnd']])
            ) {
                if (this.textCommands[ck['toFnd']][0] == '::DELETE') {
                    if (getAll) {
                    ...
                    chKeys.forEach((c) => {
                        if (
                            this.textCommands[c + '::' + cmdKey] &&
                            Array.isArray(this.textCommands[c + '::' + cmdKey])
                        ) {
                ...
                if (useChKeys.length > 0) {
                    ...
                    this.textCommands[ck['toFnd']].forEach((m) => {
                        if (callback) {
    ...
    sendMsgArrayPrep(channelKey, list, privPartRef = null) {
        ...
        if (
            (cChan && cChan['channelIrc'] && !privPartRef) ||
            (privPartRef && privPartRef.type == 1)
        ) {
            this.sendMsgArray(channelKey, list, 0, privPartRef, {
                allowIrc: true,
            });
        }

        if (
            (cChan && cChan['channelDiscord'] && !privPartRef) ||
            (privPartRef && privPartRef.type == 0)
        ) {
            var disMsg = '',
                disTmp,
                disList = [];
            ...
            disList.forEach((dm) => {
                if (privPartRef) {
                    privPartRef.noticeMessage(this, dm);
                } else {
                    const msgStr = new discord.RichEmbed();
