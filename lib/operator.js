...
class Operator {
    constructor(botRef, options) {
        ...
        this.cmds = {
            ...
            'stats' : {
                'auth' : 0, 'inst' : new OpNewStats(this), 'banable' : false,
                'info' : 'Display statistics of specific player. Usage: !stats [pug] playername'
            },
            'mystats' : {
                'auth' : 0, 'inst' : new OpNewStats(this), 'banable' : false,
            ...
            'oldstats' : {
                ...
                'info' : 'Display old pug statistics of specific player. Usage: !stats [pug] playername'
            },
            'myoldstats' : {
    ...
    loadState() {
        ...
        if (fs.existsSync("conf/persistent.json") && (data = fs.readFileSync('conf/persistent.json', 'utf8')) != false && (dt = JSON.parse(data))) {

            this.fromJSON(dt);

        }

        if (fs.existsSync("conf/config_live.json") && (data = fs.readFileSync('conf/config_live.json', 'utf8')) != false && (dt = JSON.parse(data))) {
    ...
    supplyCommandFn(channelKey, partRef, cStk, privMsg) {
        ...
        } else if ((tt = this.getCmdAuth(channelKey, this.currentCmd)) && partRef.authLevel < tt['result']) {
    ...
    removePlayerFromAllOtherCatalogs(restCat, cPartRef) {
        var acRef, signedPugs = this.getCatsInChannel(false).filter(cat => cat.flag != restCat.flag && cat.isPlayer(cPartRef));

        signedPugs.forEach(catRef => {

            catRef.leaveParticipant(cPartRef);

            if ((acRef = this.getAction(catRef.channelKey)) != null && acRef.logicState != 0 && acRef.channelKey != restCat.channelKey) {
    ...
    getUser(nick, callback, preferType, allowFromStats) {
        ...
        } else if (typeof preferType != 'undefined' && preferType !== false && preferType == 0) {
            // prefer discord
        ...
        } else if (typeof preferType != 'undefined' && preferType !== false && preferType == 1) {
            // prefer irc
    ...
    getUserListCall(nickList, idx, result, callback, allowFromStats) {
        if (idx < nickList.length) {
            this.getUser(nickList[idx], (cPartRef) => {
                ...
                if (cPartRef && result.filter(cRef => cRef.compareEqual(cPartRef)).length == 0) {
                    result.push(cPartRef);
                }

                this.getUserListCall(nickList, idx + 1, result, callback, allowFromStats);

            }, false, allowFromStats);
