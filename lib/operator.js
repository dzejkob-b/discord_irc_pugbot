...
class Operator {
    constructor(botRef, options) {
        ...
        this.cmds = {
            ...
            'list' : { 'alt' : ['ls'], 'auth' : 0, 'info' : 'List all players which are joined to the pug. Usage: !list [pug]' },
    ...
    leaveEvent(partRef) {
        this.supplyCommandAny(partRef, "lva");
    }
    ...
    getCmdAuth(cmd) {
        for (var key in this.cmds) {
            if (typeof this.cmds[key]['auth'] == 'undefined') {
                // none ...

            } else if (key == cmd) {
                return this.cmds[key]['auth'];
    ...
    supplyCommand(partRef, text) {
        if (text.startsWith("!") || text.startsWith(".")) {
            ...
            if (this.logicState == 0) {
                ...
                this.supplyCommandAny(partRef, text);
    ...
    supplyCommandAny(partRef, text) {

        let cStk = new CmdStack(this, text);

        partRef.getCompleted(this, (finPartRef) => {

            this.supplyCommandFn(finPartRef, cStk);

        });
    }

    supplyCommandFn(partRef, cStk) {

        let cRef, catRef, wRef, cmdName;
        let result;
        let teamRef;
        ...
        switch (currentCmd) {
                    ...
                    this.getUser(cStk.pop(), (cPartRef) => {
                        ...
                        if (cPartRef == null) {
                            ...
                            if (result == -1) {
                                ...
                                // msg to all
                                this.sendMsg(false, WordCo.cre().text('Player ').texth(cPartRef.nick).text(' was added to ').texth(catRef.flag).text(' pug.'));
                            }
            ...
            case 'delplayer' :
                if ((catRef = this.getCatRef(cStk.popMod())) == null) {
                    ...
                    if (result == -1) {
                        ...
                        // msg to all
                        this.sendMsg(false, WordCo.cre().text('Player ').texth(cRef.nick).text(' was removed from ').texth(catRef.flag).text(' pug.'));

                    } else {
                        // msg to all
                        this.sendMsg(false, WordCo.cre().text('The ').texth(catRef.flag).text(' pug stopped because player ').texth(cRef.nick).text(' was removed.'));
            ...
            case 'l' :
                if ((catRef = this.getCatRef(cStk.popMod())) == null) {
                    ...
                    if (result == -1) {
                        ...
                        // msg to all
                        this.sendMsg(false, WordCo.cre().text('The ').texth(catRef.flag).text(' pug stopped because player ').texth(partRef.nick).text(' left.'));
            ...
            case 'ls' :
                if (this.logicState == 2) {
                    this.sendMsg(false, this.gameRef.restCat.addStatusReadable(WordCo.cre(), true));
                    ...
                    if ((catRef = this.getCatRef(cStk.last())) == null) {
                        ...
                        this.sendMsg(false, catRef.addStatusReadable(WordCo.cre()));
                    }
            ...
            case 'teams' :
                if (this.logicState == 2) {
                    this.gameRef.teams.forEach((teamRef) => {
                        this.sendMsg(false, teamRef.addStatusReadable(WordCo.cre()));
                    });
                }
                break;
            ...
            case 'promote' :
                if (this.logicState == 0 && (cStk.last() == 'p' || cStk.last() == 'promote')) {
                    ...
                    if ((catRef = this.getCatRef(cStk.popMod())) == null) {
                        ...
                        // msg to all
                        this.sendMsg(false, WordCo.cre().text('Only ').texth(catRef.playerLimit - catRef.list.length, true).text(' needed for the ').texth(catRef.flag).text(' pug!'));
                    }
            ...
            case 'lastttt' :
                ...
                if (hSteps >= 0 && hSteps < this.gameHistory.length) {
                    ...
                    // msg to all
                    this.sendMsg(false, hGameRef.addStatusReadable(WordCo.cre(), true));

                } else {
                    partRef.noticeMessage(this, WordCo.cre().text('No such game history.'));
                }
                break;

            case 'reset' :
                if (this.logicState == 0) {
                    this.sendMsg(false, WordCo.cre().text('Cannot reset - no picking started!'));

                } else if ((catRef = this.getCatRef(this.gameRef.restCat.flag)) == null) {
                    this.sendMsg(false, WordCo.cre().text('No such pug ').texth(this.gameRef.restCat.flag).text('!'));

                } else {
                    this.startSelectCaptains(catRef, -5);

                    // msg to all
                    this.sendMsg(false, WordCo.cre().text('The picking in ').texth(catRef.flag).text(' pug was reset.'));
                }
                break;

            case 'fullreset' :
                if ((catRef = this.getCatRef(cStk.popMod())) == null) {
                    this.sendMsg(false, WordCo.cre().text('No such pug ').texth(cStk.last()).text('!'));
                    ...
                    // msg to all
                    this.sendMsg(false, WordCo.cre().text('The ').texth(catRef.flag).text(' pug was reset.'));
                }
                break;
            ...
            case 'quickpug' :
                ...
                if ((new RegExp("^[a-z]{1}[a-z0-9]{0,9}$")).test(in_flag) == false) {
                    ...
                    // msg to all
                    this.sendMsg(false, WordCo.cre().text('Pug ').texth(in_flag).text(' with ').texth(in_players).text(' players and ').texth(in_teams).text(' teams was created!'));
                    this.sendMsg(false, WordCo.cre().text('Type .j ').texth(in_flag).text(' to join this pug.'));
            ...
            case 'deletepug' :
                if ((catRef = this.getCatRef(cStk.popMod())) == null) {
                    ...
                    // msg to all
                    this.sendMsg(false, WordCo.cre().text('The ').texth(catRef.flag).text(' pug was deleted!'));
                }
                break;
            ...
            case 'userinfo' :
                ...
                if ((subPartRef = this.botRef.channelDisUsers.getUser(cStk.pop())) != null) {
                    subPartRef.getCompleted(this, (cPartRef) => {
                        ...
                        this.sendMsg(false, WordCo.cre().text(cPartRef.readableInfo()));
                ...
                if (((subPartRef = this.botRef.channelIrcUsers.getUser(cStk.last()))) != null) {
                    subPartRef.getCompleted(this, (cPartRef) => {
                        ...
                        this.sendMsg(false, WordCo.cre().text(cPartRef.readableInfo()));
                        this.sendMsg(false, WordCo.cre().text('Whois: ').texth(JSON.stringify(cPartRef.whois)));
            ...
            case 'noun' :
                this.sendMsg(false, WordCo.cre().text(this.nounRef.getNoun()));
                break;
                ...
                if ((catRef = this.getCatRef(cStk.popMod())) == null) {
                    ...
                    this.getUser(cStk.pop(), (cPartRef) => {
                        ...
                        this.statsRef.getStatsMessages(catRef.flag, cPartRef == null ? cStk.last() : cPartRef, cStk.pop(), (msgs) => {
                            ...
                            if (msgs.length > 0) {
                                this.sendMsgArray(false, msgs, 0);
                            } else {
            ...
            case 'pid' :
                this.sendMsg(false, WordCo.cre().text(process.pid));
                break;

            case 'eatstats' :
                this.statsRef.eatIni(cStk.pop(), (msg) => {

                    this.sendMsg(false, WordCo.cre().text(msg));
            ...
            case 'flushstats' :
                this.statsRef.flushStats(() => {
                    ...
                    this.sendMsg(false, WordCo.cre().text("Stats were flushed."));
    ...
    gameHasFinished() {
        if (this.logicState == 2 || this.logicState == 1) {
            ...
            this.gameHistory[0] = this.gameRef;
    ...
    sendMsgArray(channel, list, idx) {
        ...
        if (idx + 1 < list.length) {
            setTimeout(() => {
                this.sendMsgArray(channel, list, idx + 1);
            }, 1000);
        }
    }
}
