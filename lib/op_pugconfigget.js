...
class OpConfigPugGet {
    ...
    exec(channelKey) {
        ...
        switch (subCmdLowerCase) {
            case 'plcooldown':
                operRef.msgRef.sendMsg(channelKey, WordCo.cre().text(Catalog.SUBCOMMANDS[subCmd]['readable']).text(' is set to: ').text(catRef.playerRejoinCooldownInSecs, true).text(' seconds.'), privPartRef);
                break;

            case 'cptcooldown':
                operRef.msgRef.sendMsg(channelKey, WordCo.cre().text(Catalog.SUBCOMMANDS[subCmd]['readable']).text(' is set to: ').text(catRef.captainIdleCooldownInSecs, true).text(' seconds.'), privPartRef);
                break;

            case 'cptidle':
                operRef.msgRef.sendMsg(channelKey, WordCo.cre().text(Catalog.SUBCOMMANDS[subCmd]['readable']).text(' is set to: ').text(catRef.captainIdleInSecs, true).text(' seconds.'), privPartRef);
                break;

            case 'votes' :
                operRef.msgRef.sendMsg(channelKey, WordCo.cre().text(Catalog.SUBCOMMANDS[subCmd]['readable']).text(' is set to: ').text(catRef.possibleVotes).text('.'), privPartRef);
                break;

            case 'picksteps' :
                operRef.msgRef.sendMsg(channelKey, WordCo.cre().text(Catalog.SUBCOMMANDS[subCmd]['readable']).text(' is set to: ').text(catRef.pickSteps).text('.'), privPartRef);
                break;

            case 'limitnocapttag' :
                operRef.msgRef.sendMsg(channelKey, WordCo.cre().text(Catalog.SUBCOMMANDS[subCmd]['readable']).text(' is set to: ').text(catRef.limitNoCaptTag).text('.'), privPartRef);
                break;
                
            case 'avgpickmth' :
                operRef.msgRef.sendMsg(channelKey, WordCo.cre().text(Catalog.SUBCOMMANDS[subCmd]['readable']).text(' is set to: ').text(catRef.avgPickStatsMethod).text('.'), privPartRef);
                break;
