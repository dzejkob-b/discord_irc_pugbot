    ...
    exec(channelKey) {
        ...
        if (!catRef) {
            ...
            operRef.sendMsg(
                channelKey,
                WordCo.cre().text(
                    `Unknown subcommand "${subCmd}", SubCommands: ${OpConfigPugGet.SubCommandsList()}`
                ),
                privPartRef
            )
        ...
        switch (subCmdLowerCase) {
            ...
            default:
                ...
                wc.text('Is quick: ')
                    ...
                    .text(', ')
                wc.text('Player limit: ')
                    .text(catRef.playerLimit, true)
                    .text(', ')
                ...
                for (var key in Catalog.SUBCOMMANDS) {
                    wc.text(', ')
                    wc.text(
                        Catalog.SUBCOMMANDS[key]['readable'] +
                            ' [' +
                            key +
                            ']: '
                    ).text(catRef[Catalog.SUBCOMMANDS[key]['key']], true)
                }
