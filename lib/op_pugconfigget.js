...
class OpConfigPugGet {
    ...
    exec(channelKey) {
        ...
        switch (subCmdLowerCase) {
            ...
            case 'limitnocapttag' :
                operRef.sendMsg(channelKey, WordCo.cre().text(Catalog.SUBCOMMANDS[subCmd]['readable']).text(' is set to: ').text(catRef.limitNoCaptTag).text('.'), privPartRef);
                break;
            ...
            default:
                ...
                wc.text('LimitNoCaptTag: ').text(catRef.limitNoCaptTag, true);
