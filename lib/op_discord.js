...
class OpDiscord {
    ...
    exec(channelKey) {
        let operRef = this.parent, wRef, isStart, cStk = this.parent.cStk, partRef = this.parent.partRef, privPartRef = this.parent.privPartRef;

        if (!operRef.botRef.discord || operRef.currentCmd == 'discord_auth') {

            var authLink = 'https://discordapp.com/oauth2/authorize?client_id=' + (operRef.botRef.discordClientId ? operRef.botRef.discordClientId : '[YOUR_CLIENT_ID]') + '&scope=bot';
