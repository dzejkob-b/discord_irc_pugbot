...
class OpCreatePug {
    ...
    exec(channelKey) {
        ...
        if ((new RegExp("^[a-z0-9]{1}[a-z0-9]{0,9}$", 'i')).test(in_flag) == false) {
            ...
            // msg to all
            operRef.msgRef.sendMsg(channelKey, WordCo.cre().text('Pug ').texth(in_flag).text(' with ').texth(in_players).text(' players and ').texth(in_teams).text(' teams was created!'), privPartRef);
            operRef.msgRef.sendMsg(channelKey, WordCo.cre().text('Type .j ').texth(in_flag).text(' to join this pug.'), privPartRef);
