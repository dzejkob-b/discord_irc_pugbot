	...
	exec(channelKey) {
		...
		if (new RegExp("^[a-z0-9]{1}[a-z0-9]{0,9}$", "i").test(in_flag) == false) {
			partRef.noticeMessage(
				operRef,
				WordCo.cre().text(
					"Invalid pug name! Use only characters and numbers (max 10 characters)."
				)
			);
		} else if (in_players < 1 || in_players > 20) {
			partRef.noticeMessage(
				operRef,
				WordCo.cre().text("Player count must be between 1-20!")
			);
		} else if ((in_teams == 0 || (in_teams >= 2 && in_teams <= 4)) == false) {
			partRef.noticeMessage(
				operRef,
				WordCo.cre().text("Team count must be between 2-4 or 0!")
			);
		} else if (in_teams != 0 && in_players % in_teams != 0) {
			partRef.noticeMessage(
				operRef,
				WordCo.cre().text("Player count must be divisible by team count!")
			);
			...
			partRef.noticeMessage(
				operRef,
				WordCo.cre().text("Pug ").texth(in_flag).text(" allready exists!")
			);
		} else if (
			is_quick &&
			partRef.authLevel < 10 &&
			operRef.getCatRefByCreator(partRef) != null
		) {
			partRef.noticeMessage(
				operRef,
				WordCo.cre().text("You allready have created one quick pug!")
			);
		} else {
			...
			operRef.msgRef.sendMsg(
				...
				WordCo.cre()
					...
					.text(" players and ")
					.texth(in_teams)
					.text(" teams was created!"),
				privPartRef
			);
			operRef.msgRef.sendMsg(
				channelKey,
				WordCo.cre().text("Type .j ").texth(in_flag).text(" to join this pug."),
				privPartRef
			);
		}
	}
