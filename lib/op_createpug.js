...
class OpCreatePug {
    ...
    exec() {
        ...
        if ((new RegExp("^[a-z0-9]{1}[a-z0-9]{0,9}$")).test(in_flag) == false) {
        ...
        } else if (operRef.getCatRef(in_flag) != null) {
            ...
            // msg to all
            operRef.sendMsg(false, WordCo.cre().text('Pug ').texth(in_flag).text(' with ').texth(in_players).text(' players and ').texth(in_teams).text(' teams was created!'), privPartRef);
            operRef.sendMsg(false, WordCo.cre().text('Type .j ').texth(in_flag).text(' to join this pug.'), privPartRef);
