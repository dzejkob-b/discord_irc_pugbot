...
class Catalog {
    ...
    static DEFAULT_CAPTAIN_IDLE_IN_SECS = 180;
    ...
    constructor(
        ...
        this.possibleVotes =
            this.extraSettings.possibleVotes || Catalog.DEFAULT_POSSIBLE_VOTES;
        this.captainIdleInSecs =
            this.extraSettings.captainIdleInSecs ||
            Catalog.DEFAULT_CAPTAIN_IDLE_IN_SECS;
        this.captainIdleCooldownInSecs =
            this.extraSettings.captainIdleCooldownInSecs ||
            Catalog.DEFAULT_CAPTAIN_IDLE_COOLDOWN_IN_SECS;
        this.playerRejoinCooldownInSecs =
            this.extraSettings.playerRejoinCooldownInSecs ||
        ...
        this.list.forEach((partRef) => {
            ...
            while (!brk && idx < result.length) {
                ...
                var rKoef = result[idx]['partRef'].avgTotPick
                    ? result[idx]['partRef'].avgPick /
                      result[idx]['partRef'].avgTotPick
                    : 10000;
    ...
    loadAvgPick(statsRef, callback) {
        if (!this.list || this.list.length == 0) {
            ...
            statsRef.sumRef.getStatsMultiple(this.flag, this.list, function (
                rows
            ) {
                if (rows) {
                    rows.forEach((row) => {
                        ...
                        if (brk) {
                            ...
                            if (tPugs > 0) {
                                self.list[pos].avgPick =
                                    Math.round((row['picks'] / tPugs) * 100.0) /
                                    100.0;
                                self.list[pos].avgTotPick =
                                    Math.round(
                                        (row['totalpicks'] / tPugs) * 100.0
                                    ) / 100.0;
                            }
                        }
                    });
    ...
    addStatusReadable(wRef, useForceIndex, voteRef, channelKey, operRef) {
        ...
        if (channelKey && operRef && this.channelKey != channelKey) {
            wRef.channelLink(
                operRef.botRef.channels[this.channelKey],
                '{',
                '}'
            );
        }
            ...
            while (idx < sorted.length) {
                ...
                if (
                    ...
                    var koef = Math.round(
                        (sorted[idx]['partRef'].avgTotPick
                            ? 1.0 -
                              (sorted[idx]['partRef'].avgPick - 1.0) /
                                  (sorted[idx]['partRef'].avgTotPick - 1.0)
                            : 0) * 100.0
                    );
    ...
    getParticipantNickOrForceIndex(input) {
        ...
        while (idx < this.list.length) {
            if (
                this.list[idx].nick.toLowerCase().trim() ==
                    input.toLowerCase() ||
    ...
    joinParticipantSorted(partRef) {
        ...
        if (idx != -1 || !partRef) {
            ...
            while (!found && pos < this.list.length) {
                if (this.list[pos].forceIndex > partRef.forceIndex)
                    found = true;
                else pos++;
            }
    ...
    leaveParticipantCooldown(
        ...
        if (res == 0 && gameRef.areCaptainsSet) {
    ...
    getRandomParticipant() {
        if (this.list.length == 0) {
            ...
            return this.list[
                Math.round(Math.random() * (this.list.length - 1))
            ];
        }
    }
    ...
    testParticipantTimeout(partRef) {
        ...
        if (idx != -1) {
            let restTime =
                this.timeouts[idx]['time'] - new Date().getTime() / 1000;
    ...
    addParticipantTimeout(
        ...
        if (secs) {
            ...
            cTime +=
                this.playerRejoinCooldownInSecs || operRef.botRef.rejoinTimeout;
        }
