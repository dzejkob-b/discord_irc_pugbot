...
class Catalog {
    ...
    getClone() {
        var catRef = new Catalog(this.channelKey, this.flag, this.playerLimit, this.teamCount, this.parentCatRef, this.getExtraSettings());
    ...
    canPromote() {
        return !this.lastPromoteTime || !TimeUtils.lessThenInSecs(this.lastPromoteTime, 10*60*1000) || (this.lastPromotePlayersCount || 0) < this.list.length;
    }
    ...
    loadAvgPick(statsRef, method, callback) {
        ...
        if (!method) {
            ...
            logger.debug("Avg pick method for " + this.flag + " is:" + this.avgPickStatsMethod + " (" + JSON.stringify(method) + ")");
        }

        if (!method) {
            method = statsRef.parseAvgStatMethod('sumarize');
            logger.debug("Avg pick method for " + this.flag + " is default: " + JSON.stringify(method));
        }
        ...
        if (this.list.length == 0) {
            ...
            while (idx < sorted.length) {
                ...
                if (sorted[idx]['partRef'].avgTotPick && sorted[idx]['partRef'].avgTotPick > 1) {
                    ...
                    var koef = Math.round((sorted[idx]['partRef'].avgTotPick ? (1.0 - (( - 1.0) / (sorted[idx]['partRef'].avgTotPick - 1.0))) : 0) * 100.0);

                    wRef.text(sorted[idx]['partRef'].avgPick, true);
                    wRef.text('/');
                    wRef.text(sorted[idx]['partRef'].avgTotPick);
                    wRef.texth(']');
