...
class Catalog {
    ...
    addStatusReadable(wRef, useForceIndex, voteRef, channelKey, operRef, showStats = false) {
        ...
        if (this.list.length == 0) {
            ...
            while (idx < this.list.length) {
                ...
                if (showStats && this.list[idx].avgTotPick) {
                    wRef.texth('[');
                    wRef.text('P:');
                    wRef.text(this.list[idx].avgPick).texth('/').text(this.list[idx].avgTotPick);
                    wRef.texth(']');
                }
