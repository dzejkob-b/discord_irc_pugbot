    ...
    exec(channelKey) {
        ...
        if (operRef.logicState == 0 || operRef.logicState == 1) {
            ...
            if (tt['partRef'] == null) {
                ...
                tt = operRef.voteRef.getParticipantForVoteNickOrForceIndex(
                    cStk.pop()
                )
                ...
                if (tt['partRef'] == null) {
                    ...
                    let voteResult = operRef.voteRef.addVote(
                        operRef,
                        tt['catRef'],
                        partRef,
                        votePartRef,
                    ...
                    if (voteResult >= 0 && operRef.logicState == 0) {
                        ...
                        wRef.text('User ')
                            ...
                            .texth('+' + voteValue)
                            .text(' votes (total ')
                            .texth(operRef.voteRef.getTotalVotes(votePartRef))
                            .text(' votes). ')

                        if (votePartRef.isNocapt()) {
                            wRef.texth(votePartRef.nick)
                        ...
                        partRef.noticeMessage(
                            operRef,
                            WordCo.cre().text(
                                'You cannot vote for nocapt users!'
                            )
                        )
                    }
                }
            ...
            partRef.noticeMessage(
                operRef,
                WordCo.cre().text(
                    'You can vote for captain only when pug is filled!'
                )
            )
        }
    }
