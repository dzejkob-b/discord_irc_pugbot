		...
		if (acRef.logicState == 0 || acRef.logicState == 1) {
			...
			if (tt["partRef"] == null) {
				partRef.noticeMessage(
					operRef,
					WordCo.cre()
						.text("You cannot vote you are not joined in the ")
						.texth(tt["catRef"].flag)
						.text(" pug!")
				);
				...
				tt = acRef.voteRef.getParticipantForVoteNickOrForceIndex(
					channelKey,
					cStk.pop()
				);
				...
				if (tt["partRef"] == null) {
					...
					let voteResult = acRef.voteRef.addVote(
						operRef.getAction(channelKey),
						tt["catRef"],
						partRef,
						votePartRef,
						voteValue
					...
					if (
						...
						wRef
							.text("User ")
							.texth(votePartRef.nick)
							.text(" gained ")
							.texth("+" + voteValue)
							.text(" votes (total ")
							.texth(acRef.voteRef.getTotalVotes(votePartRef))
							.text(" votes). ");
						...
						if (votePartRef.isNocapt()) {
							wRef
								...
								.texth("nocapt")
								.text(" - was voted for you.");
						}

						operRef.msgRef.sendMsg(channelKey, wRef, privPartRef);
					} else if (
						voteResult >= 0 &&
						operRef.getAction(channelKey).logicState == 1
					) {
						// pug is filled
						...
						partRef.noticeMessage(
							operRef,
							WordCo.cre().text(
								"You have exhausted the number of possible votes!"
							)
						);
					} else {
						partRef.noticeMessage(
							operRef,
							WordCo.cre().text("You cannot vote for nocapt users!")
						);
					}
				}
			}
			...
			partRef.noticeMessage(
				operRef,
				WordCo.cre().text("You can vote for captain only when pug is filled!")
			);
		}
	}
}
