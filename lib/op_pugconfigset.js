...
class OpConfigPugSet {
	...
	exec(channelKey) {
		...
		if (!catRef) {
			...
			!subCmd ||
			!Catalog.SUBCOMMANDS[subCmd] ||
			!Catalog.SUBCOMMANDS[subCmd]["settable"]
		) {
			if (!subCmd) {
				operRef.msgRef.sendMsg(
					channelKey,
					WordCo.cre().text(
						`Please specify subcommand to set: ${Catalog.SubCommandsList()}`
					),
					privPartRef
				);
			} else {
		...
		switch (subCmdLowerCase) {
			...
			case "picksteps":
				...
				if (invalidMsg) {
					operRef.msgRef.sendMsg(
						channelKey,
						WordCo.cre().text(invalidMsg),
						privPartRef
					);
				} else if (finSteps.length == 0) {
					operRef.msgRef.sendMsg(
						channelKey,
						WordCo.cre().text("No picking steps specified!"),
						privPartRef
					);
				} else {
				...
				if (parseInt(value) > maxNoCapt) {
					operRef.msgRef.sendMsg(
						...
						WordCo.cre()
							.text(Catalog.SUBCOMMANDS[subCmd]["readable"])
							.text(
								` value exceed maximum allowed for the pug. For disabling set the value of '0'`
							),
						privPartRef
					);
			...
			case "isdefault":
				const isDefaultRequest = value === "true";
				catRef.setIsDefault(isDefaultRequest);
			...
			default:
				operRef.msgRef.sendMsg(
					channelKey,
					WordCo.cre().text(
						`You must supply a subcommand ${Catalog.SubCommandsList()}`
					),
					privPartRef
				);
				break;
