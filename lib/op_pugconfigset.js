        ...
        if (!catRef) {
            ...
            if (!subCmd) {
                operRef.sendMsg(
                    channelKey,
                    WordCo.cre().text(
                        `Please specify subcommand to set: ${OpConfigPugSet.SubCommandsList()}`
                    ),
                    privPartRef
                )
            } else {
                operRef.sendMsg(
                    channelKey,
                    WordCo.cre().text(
                        `Unknown subcommand "${subCmd}", SubCommands: ${OpConfigPugSet.SubCommandsList()}`
                    ),
                    privPartRef
                )
            }
        ...
        switch (subCmdLowerCase) {
            ...
            default:
                operRef.sendMsg(
                    channelKey,
                    WordCo.cre().text(
                        `You must supply a subcommand ${OpConfigPugSet.SubCommandsList()}`
                    ),
                    privPartRef
                )
                break
