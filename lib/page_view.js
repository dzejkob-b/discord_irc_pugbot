...
class PageView {
    ...
    doComposeLoop(idx, cont, dt) {
        ...
        this.operRef.statsRef.getGameStat(false, false, false, idx, function(st_rows, pl_rows) {
    ...
    doComposeFinal(cont, dt) {
        var wRef, sf, fs = require('fs');

        this.operRef.cats.forEach((catRef) => {

            wRef = catRef.addStatusReadable(WordCo.cre(), false, this.operRef.getAction(catRef.channelKey).voteRef);
