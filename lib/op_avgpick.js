...
class OpAvgPick {
	...
	exec(channelKey) {
		...
		if (
			operRef.currentCmd != "avgpickspec" &&
			...
			if (catRef != null) {
				...
				if (lastPop) {
					partRef.noticeMessage(
						operRef,
						WordCo.cre().text("No such pug ").texth(lastPop).text("!")
					);
				} else {
					partRef.noticeMessage(
						operRef,
			...
			if (
				catRef != null &&
				(operRef.currentCmd == "avgpickspec" || (lastPop = cStk.pop()))
			) {
				if (lastPop) {
					cStk.add(lastPop);
				}
					...
					(users) => {
						if (users.length > 0) {
							...
							nCatRef.loadAvgPick(operRef.statsRef, mth, function (aMth) {
								tmList.push(
									nCatRef.addStatusReadableAvgPick(
										aMth,
										WordCo.cre(),
										false,
										acRef.voteRef,
