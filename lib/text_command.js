...
class TextCommand {
    ...
    isCommandExist(channelKey, cmdKey, allowDelete = true) {
        ...
        for (let ck of [channelKey + "::" + cmdKey, cmdKey]) {
            if (this.operRef.textCommands[ck] && Array.isArray(this.operRef.textCommands[ck])) {
    ...
    getTextCommand(channelKey, cmdKey, getAll = false, callback = false, callbackData = false) {
        ...
        for (let ck of lst) {
            if (this.operRef.textCommands[ck['toFnd']] && Array.isArray(this.operRef.textCommands[ck['toFnd']])) {
                ...
                if (this.operRef.textCommands[ck['toFnd']][0] == "::DELETE") {
                    ...
                    chKeys.forEach((c) => {
                        if (this.operRef.textCommands[c + '::' + cmdKey] && Array.isArray(this.operRef.textCommands[c + '::' + cmdKey])) {
