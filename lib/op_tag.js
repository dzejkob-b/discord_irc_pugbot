...
class OpTag {
	...
	exec(channelKey) {
		...
		if (!operRef.anyCats(channelKey)) {
			...
			if (pTag.toLowerCase() == "nocapt") {
				const participantsWithNoCapt = catRef.getParticipantsWithTag("nocapt");
				if (!catRef.limitNoCaptTag) {
					partRef.noticeMessage(
						operRef,
						WordCo.cre().text(
							`You cannot add the nocapt tag. NoCapt tag is disabled.`
						)
					);
					return;
				} else if (
					!participantsWithNoCapt.includes(partRef) &&
					participantsWithNoCapt.length == catRef.limitNoCaptTag
				) {
					partRef.noticeMessage(
						operRef,
						WordCo.cre().text(
							`You cannot add the nocapt tag. Only ${catRef.limitNoCaptTag} nocapt allowed!`
						)
					);
					return;
				}
