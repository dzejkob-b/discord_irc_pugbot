...
class OpTag {
    ...
    exec(channelKey) {
        let operRef = this.parent, catRef, cRef, wRef, cStk = this.parent.cStk, partRef = this.parent.partRef;
        ...
        if (!operRef.anyCats(channelKey)) {
            ...
            if (operRef.limitNoCaptTag > 0) {
                ...
                if (!participantsWithNoCapt.includes(partRef) && participantsWithNoCapt.length == operRef.limitNoCaptTag) {
                    partRef.noticeMessage(operRef, WordCo.cre().text("You cannot add the nocapt tag. Only " + operRef.limitNoCaptTag + " nocapt allowed!"));
                    return;
                }
            }
