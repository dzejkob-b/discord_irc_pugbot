        ...
        if (operRef.currentCmd == 'userinfo2') {
            operRef.getUser(cStk.pop(), (cPartRef) => {
                if (cPartRef != null) {
                    ...
                    operRef.sendMsg(false, WordCo.cre().text(cPartRef.readableInfo()), privPartRef);
            ...
            if (((subPartRef = operRef.botRef.channelIrcUsers.getUser(cStk.last()))) != null) {
                subPartRef.getCompleted(operRef, (cPartRef) => {
                    ...
                    operRef.sendMsg(false, WordCo.cre().text(cPartRef.readableInfo()), privPartRef);
                    operRef.sendMsg(false, WordCo.cre().text('Whois: ').texth(JSON.stringify(cPartRef.whois)), privPartRef);
