...
class OpDeletePug {
    ...
    exec(channelKey) {
        ...
        if (!operRef.anyCats(channelKey)) {
        ...
        } else if ((operRef.getAction(channelKey).gameRef == null || catRef.flag == operRef.getAction(channelKey).gameRef.restCat.flag) && operRef.getAction(channelKey).logicState != 0) {
            ...
            // msg to all
            operRef.msgRef.sendMsg(channelKey, WordCo.cre().text('The ').texth(catRef.flag).text(' pug was deleted!'), privPartRef);
        }
    }
}
