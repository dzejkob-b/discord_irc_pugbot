	...
	exec(channelKey) {
		...
		if (!operRef.anyCats(channelKey)) {
			...
			if (cStk.first()) {
				...
				partRef.noticeMessage(
					operRef,
					WordCo.cre().text("Specify pug for trend stats!")
				);
			}
		} else {
			...
			operRef.statsRef.getTrendStats(
				catRef,
				TimeUtils.getDaysList(dNum),
				(result) => {
					var x_sum = 0;
					var x_sum_squares = 0;
					var y_sum = 0;
					...
					var a_div = idx_hash_length * x_sum_squares - Math.pow(x_sum, 2),
						a,
						b,
						c;
					...
					wRef
						.text(" count of daily pugs trend for past ")
						.texth(dNum)
						.text(" days is: <from: ");
